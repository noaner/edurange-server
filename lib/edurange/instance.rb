  ...
  class Instance
    ...
    def startup
      ...
      if @is_nat
        ...
        Dir.chdir(Settings.chef_path + '/cookbooks/') do
          ...
          Dir.chdir(Dir.getwd + cookbook_path) do
            ...
            File.open('metadata.rb', 'w') do |file|
              ...
              file.puts "%w{ ubuntu debian redhat centos fedora freebsd}.each do { |os| supports os }"
            end
          ...
          `knife upload /cookbooks` # Upload all changed cookbooks
        end
        exit
        ...
        sleep_until_running
        info "Waiting 3 minutes before SSHing into instance."
        sleep 180
        ...
        knife_command = "knife bootstrap #{tmp_instance_ip} -N #{@name} --ssh-user ubuntu --sudo --run-list 'recipe[edurange_base],recipe[edurange_bootstrap_#{@subnet.cloud.vpc_id}_#{@name}]'"
