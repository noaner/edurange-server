...
module Aws
  ...
  def aws_boot_instance
    ...
    # some sort of race condition maybe where ec2instance is nil?
    sleep 2

    # set instance driver_id
    debug "assigning - Instance #{self.name} driver_id"
    begin
  ...
  handle_asynchronously :aws_boot_instance, queue: "instances"
  ...
  handle_asynchronously :aws_unboot_instance, :priority => 0
  ...
  def aws_upload_com_page(scenario)
    begin
      s3 = AWS::S3.new
      bucket = s3.buckets[Settings.bucket_name]
      s3.buckets.create(Settings.bucket_name) unless bucket.exists?
  ...
  def aws_scenario_upload_com_page
    begin
      s3 = AWS::S3.new
      bucket_name = Settings.bucket_name + '-edurange-com'
